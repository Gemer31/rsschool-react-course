...
export const organizationAPI = createApi({
    ...
    endpoints: (build) => ({
        fetchOrganizations: build.query<IOrganizationsResponse,
        ...
        fetchOrganizationDetails: build.query<IOrganizationsResponse, string>({
            ...
            async onQueryStarted(_arg, { dispatch, queryFulfilled }) {
                const response: { data: { organization: IOrganization} } = await queryFulfilled;
...
export const {
    useFetchOrganizationsQuery, useFetchOrganizationDetailsQuery,
    util: {getRunningQueriesThunk}
} = organizationAPI;
export const {fetchOrganizations, fetchOrganizationDetails} = organizationAPI.endpoints;